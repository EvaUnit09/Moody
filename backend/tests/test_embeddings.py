...
class TestEmbedText:
    ...
    async def test_returns_single_embedding(self):
        ...
        mock_client.embeddings.create = AsyncMock(return_value=_mock_response([[0.1, 0.2, 0.3]]))
        
        with patch("app.services.embeddings._get_client", return_value=mock_client):
    ...
    async def test_passes_text_and_model_to_client(self):
        ...
        mock_client = AsyncMock()
        mock_client.embeddings.create = AsyncMock(return_value=_mock_response([[0.5]]))
